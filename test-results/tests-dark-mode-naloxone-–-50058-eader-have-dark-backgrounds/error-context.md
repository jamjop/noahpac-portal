...
- Name: tests/dark-mode.spec.ts >> naloxone – dark-mode smoke test >> body and header have dark backgrounds
- Location: tests/dark-mode-helpers.ts:228:5
...
```ts
  129 |   const rgb = parseRgb(color);
  130 |   expect(rgb, `${label} bg: could not parse "${color}"`).not.toBeNull();
  131 |   const max = Math.max(...rgb!);
  132 |   const direction =
  133 |     scheme === 'dark'
  134 |       ? `too bright for dark mode (max channel ${max} ≥ 200)`
  135 |       : `too dark for light mode (max channel ${max} ≤ 100)`;
  136 |   expect(
