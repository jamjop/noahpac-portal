...
function resistCell(val) {
  ...
  if (val === "nr") return `<span class="susc-badge susc-nr">~</span>`;
  const r = 100 - val;
  const cls = r<=10?"susc-hi":r<=30?"susc-mid":"susc-lo";
  return `<span class="susc-badge ${cls}">${r}%</span>`;
}
  ...
  document.getElementById("result-area").innerHTML = `
    <div class="result-card">
      <div class="result-head">
        <i data-lucide="pill" class="icon-result"></i>
      ...
      <div class="legend-row">
        <span class="susc-badge susc-hi">≤10%</span> low resistance &nbsp;·&nbsp;
        <span class="susc-badge susc-mid">11–30%</span> moderate &nbsp;·&nbsp;
        <span class="susc-badge susc-lo">&gt;30%</span> high resistance &nbsp;·&nbsp;
        <span class="susc-badge susc-nr">~</span> not recommended &nbsp;·&nbsp;
...
async function loadFacilities() {
  document.getElementById("result-area").innerHTML = '<div class="state"><i data-lucide="loader" class="icon-state"></i> Loading antibiogram data…</div>';
  if (window.lucide) lucide.createIcons();
  ...
  try {
    ...
    document.getElementById("result-area").innerHTML =
      '<div class="state err"><i data-lucide="alert-triangle" class="icon-state"></i> Could not load antibiogram manifest. Please try again later.</div>';
    if (window.lucide) lucide.createIcons();
    return;
  }
  ...
  if (!Object.keys(FACILITY_SUSC).length) {
    document.getElementById("result-area").innerHTML =
      '<div class="state err"><i data-lucide="alert-triangle" class="icon-state"></i> Could not load antibiogram data for any facility. Please try again later.</div>';
    if (window.lucide) lucide.createIcons();
    return;
  }
  ...
  if (failedIds.length) {
    ...
    warn.innerHTML = `<i data-lucide="alert-triangle" class="icon-state"></i> <span>Data unavailable for: ${failedIds.join(', ')}. Results may be incomplete.</span>`;
  ...
  if (staleFacIds.length) {
    ...
    staleWarn.innerHTML =
      `<i data-lucide="alert-triangle" class="icon-state"></i> <span>Antibiogram data may be outdated (${staleFacIds.join(', ')}). ` +
      `The data is more than ${STALE_DAYS} days past its stated period — ` +
      `verify current resistance rates with your pharmacy or infection control team.</span>`;
