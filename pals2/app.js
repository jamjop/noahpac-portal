...
const SECTIONS = [
  ...
  // ── Pediatric VF / pVT ────────────────────────────────────────────────────
  {
    id:'vfpvt', label:'VF / pVT', icon:'zap',
    render() { return `
  ...
  {
    id:'pea', label:'Asystole / PEA', icon:'heart-crack',
    render() { return `
  ...
  {
    id:'brady', label:'Bradycardia', icon:'trending-down',
    render() { return `
  ...
  {
    id:'tachy', label:'Tachycardia', icon:'trending-up',
    render() { return `
  ...
  {
    id:'rosc', label:'Post-Arrest Care', icon:'heart-handshake',
    render() { return `
  ...
  // ── Pediatric Shock ───────────────────────────────────────────────────────
  {
    id:'shock', label:'Shock', icon:'droplets',
    render() { return `
...
// ── Nav & render (abx2 sidebar/pane + search pattern) ────────────────────────

let current = SECTIONS[0].id;

// Strip HTML tags so search matches on visible algorithm text, not markup.
function sectionText(sec) {
  return (sec.label + ' ' + sec.render()).replace(/<[^>]*>/g, ' ').toLowerCase();
}

function buildNav() {
  const q = document.getElementById('palsSearch').value.trim().toLowerCase();
  const nav = document.getElementById('tabNav');
  nav.innerHTML = '';

  const matches = SECTIONS.filter(s => !q || sectionText(s).includes(q));
...
function renderSection() {
  ...
  document.getElementById('content').innerHTML = sec ? sec.render() : '<div class="placeholder">Select an algorithm from the list</div>';
}

document.getElementById('palsSearch').addEventListener('input', buildNav);
buildNav();
renderSection();
lucide.createIcons();

// ── Offline badge (kept from original; the retirement SW in index.html handles
//    registration — no scoped /pals2/sw.js is registered for this test build) ──
