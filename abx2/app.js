...
const CATS = [
  ...
  {id:'pulmonary',   label:'Pulmonary',             icon:'wind'},
  {id:'cardiac',     label:'Cardiac',               icon:'heart-pulse'},
  {id:'gi',          label:'GI / Abdominal',        icon:'utensils'},
  {id:'gu',          label:'Genitourinary',         icon:'droplet'},
  {id:'ssti',        label:'Skin & Soft Tissue',    icon:'bandage'},
  {id:'bone',        label:'Bone & Joint',          icon:'bone'},
  {id:'bloodstream', label:'Bloodstream / Sepsis',  icon:'zap'},
  {id:'tickborne',   label:'Tick-borne',            icon:'bug'},
];
...
function buildNav() {
  ...
  for (const cat of CATS) {
    ...
    for (const a of items) {
      ...
      btn.dataset.id = a.id;
      btn.innerHTML = `<i data-lucide="${cat.icon}" class="icon-tab"></i><span>${esc(a.name)}</span>`;
...
function copyRegimen() {
  ...
  lines.push(`Source: ${a.source} — noahpac.com/abx2/`);
