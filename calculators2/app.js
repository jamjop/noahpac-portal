...
const CALCS = [
  ...
  /* 1. Accutane Dosing */
  {
    ...
    render: pane => {
      ...
      <p class="note"><strong>How the duration projection is calculated:</strong> Month 1 runs at the starting dose (0.5 mg/kg/day), contributing <em>starting dose × 30 days</em> of cumulative mg. The remaining mg needed to reach the target cumulative dose is divided by the planned daily dose to get the additional days. Total duration = 30 days + remaining days, expressed in 30-day months. The target defaults to 135 mg/kg — within the conventional range — but can be adjusted based on individual patient factors.<br><br>Example (135 mg/kg target): 46 kg patient on 40 mg/day. Target = 6,210 mg. Month 1 at 23 mg/day = 690 mg. Remaining = 5,520 mg ÷ 40 mg/day = 138 days. Total = 168 days = 5 months 18 days.<br><br><strong>References:</strong> Dosing parameters (0.5–1 mg/kg/day) are consistent with: <a href="https://pubmed.ncbi.nlm.nih.gov/26897386/" target="_blank" rel="noopener">Zaenglein AL et al. "Guidelines of care for the management of acne vulgaris." <em>J Am Acad Dermatol.</em> 2016;74(5):945–973</a> (AAD); <a href="https://pubmed.ncbi.nlm.nih.gov/1460120/" target="_blank" rel="noopener">Layton AM, Cunliffe WJ. "Guidelines for optimal use of isotretinoin in acne." <em>J Am Acad Dermatol.</em> 1992;27(6 Pt 2):S2–7</a>; and <a href="https://pubmed.ncbi.nlm.nih.gov/39813053/" target="_blank" rel="noopener">Lai J, Barbieri JS. "Acne Relapse and Isotretinoin Retrial in Patients With Acne." <em>JAMA Dermatol.</em> 2025;161(4):367–374</a> — a retrospective cohort of 19,907 patients finding that higher cumulative dosage (conventional range 120–220 mg/kg) was independently associated with decreased relapse; daily dose was not associated with decreased relapse risk at conventional and high cumulative doses.</p>`;
...
function showCalc(id){
  ...
  // Re-render any Lucide icons injected into the pane
  if (window.lucide) lucide.createIcons();
}

/* ── Categories (grouping only — presentation; every calc keeps its own
   render/calc logic untouched). One icon per category, reused across all
   calculators in it, matching abx2/app.js's buildNav() pattern. ── */
const CAT_GROUPS = [
  { label:'Cardiovascular',    icon:'heart-pulse',   ids:['ascvd','chads','heart','rcri','gupta'] },
  { label:'Thrombosis',        icon:'droplet',       ids:['wellsdvt','wellspe'] },
  { label:'Renal / Metabolic', icon:'flask-conical', ids:['crcl','egfr','electro','meld'] },
  { label:'Pulmonary',         icon:'wind',          ids:['curb65'] },
  { label:'Mental Health',     icon:'brain',         ids:['phq9','gad7'] },
  { label:'Emergency / MSK',   icon:'bone',          ids:['alvarado','ottawa','pecarn'] },
  { label:'General',           icon:'stethoscope',   ids:['accutane','edd','weight-change'] },
];

/* ── Build category-grouped, collapsible nav (styled like the homepage's
   tool-list, mirroring abx2). ── */
...
function buildNav(){
  ...
  for(const grp of CAT_GROUPS){
    ...
    const hasActive = items.some(c => c.id === currentCalc);
    ...
    hdr.innerHTML = `<span class="nav-section-label"><i data-lucide="${grp.icon}" class="icon-cat"></i>${esc(grp.label)}</span><span class="nav-chevron"></span>`;
    ...
    for(const c of items){
      ...
      btn.className = 'tab-btn' + (currentCalc === c.id ? ' active' : '');
      btn.dataset.id = c.id;
      btn.innerHTML = `<i data-lucide="${grp.icon}" class="icon-tab"></i><span>${esc(c.name)}</span>`;
    ...
    tabNav.appendChild(section);
  }
  if (window.lucide) lucide.createIcons();
}

const _origShowCalc = showCalc;
showCalc = function(id){
  _origShowCalc(id);
  // Reflect active state + open the containing section in the grouped nav
  document.querySelectorAll('.tab-btn').forEach(b => b.classList.toggle('active', b.dataset.id === id));
  const activeBtn = document.querySelector(`.tab-btn[data-id="${id}"]`);
  const section = activeBtn && activeBtn.closest('.nav-section');
  if(section && !section.classList.contains('open')){
    section.classList.add('open');
    const hdr = section.querySelector('.nav-section-hdr');
