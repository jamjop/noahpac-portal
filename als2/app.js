...
// Per-section Lucide icons (kebab-case in data-lucide; each verified present in
// /lib/lucide.min.js as its PascalCase form). Content above is untouched — this
// map only drives the redesigned sidebar-nav presentation.
const SECTION_ICONS = {
  vfpvt:  'zap',
  pea:    'heart-off',
  brady:  'activity',
...
function buildNav() {
  const q = document.getElementById('alsSearch').value.trim().toLowerCase();
  ...
  if (items.length === 0) {
    nav.innerHTML = `<div class="nav-empty">No algorithms match.</div>`;
    return;
  }

  nav.innerHTML = items.map(s => {
    const icon = SECTION_ICONS[s.id] || 'activity';
    return `<button class="tab-btn${s.id===current?' active':''}" data-id="${esc(s.id)}"><i data-lucide="${icon}" class="icon-tab"></i><span>${esc(s.label)}</span></button>`;
  }).join('');
...
document.getElementById('alsSearch').addEventListener('input', buildNav);
buildNav();

// OFFLINE badge toggling via browser online/offline events. NOTE: unlike the
// production /als/ page, this comparison build intentionally does NOT register
// a scoped offline-caching service worker (see /sw.js retirement script in
// index.html). The badge therefore has nothing cached to serve if truly
// offline — a known, accepted gap to revisit only if als2 is promoted.
