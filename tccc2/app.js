...
const SECTIONS = [
  ...
  {
    id:'M', label:'M: Hemorrhage', color:'red', icon:'droplets', group:'MARCH PAWS Sequence',
    keywords:'massive hemorrhage tourniquet bleeding wound packing junctional truncal txa conversion',
    render() { return `
  ...
  // ── A: Airway ──────────────────────────────────────────────────────────────
  {
    id:'A1', label:'A: Airway', color:'teal', icon:'wind', group:'MARCH PAWS Sequence',
    keywords:'airway npa nasopharyngeal jaw thrust surgical cricothyroidotomy cric obstruction',
    render() { return `
  ...
  // ── R: Respiration ─────────────────────────────────────────────────────────
  {
    id:'R', label:'R: Respiration', color:'teal', icon:'activity', group:'MARCH PAWS Sequence',
    keywords:'respiration chest seal pneumothorax tension needle decompression sucking chest wound thoracostomy',
    render() { return `
  ...
  // ── C: Circulation ─────────────────────────────────────────────────────────
  {
    id:'C', label:'C: Circulation', color:'red', icon:'heart-pulse', group:'MARCH PAWS Sequence',
    keywords:'circulation iv io intraosseous shock resuscitation blood whole blood txa tranexamic calcium',
    render() { return `
  ...
  // ── H: Hypothermia / Head ──────────────────────────────────────────────────
  {
    id:'H', label:'H: Hypothermia', color:'teal', icon:'thermometer', group:'MARCH PAWS Sequence',
    keywords:'hypothermia lethal triad head injury tbi seizure midazolam avpu gcs rewarming',
    render() { return `
  ...
  // ── P: Pain ────────────────────────────────────────────────────────────────
  {
    id:'P', label:'P: Pain', color:'warn', icon:'pill', group:'MARCH PAWS Sequence',
    keywords:'pain management cwmp combat wound medication pack ketamine esketamine suzetrigine acetaminophen meloxicam',
    render() { return `
  ...
  // ── A2: Antibiotics ────────────────────────────────────────────────────────
  {
    id:'ABX', label:'A: Antibiotics', color:'ok', icon:'syringe', group:'MARCH PAWS Sequence',
    keywords:'antibiotics cefadroxil cephalexin ceftriaxone open wound prophylaxis infection',
    render() { return `
  ...
  // ── W: Wounds ──────────────────────────────────────────────────────────────
  {
    id:'W', label:'W: Wounds', color:'teal', icon:'bandage', group:'MARCH PAWS Sequence',
    keywords:'wound care evisceration eye injury open globe fox shield burns rule of nines bsa',
    render() { return `
  ...
  {
    id:'S', label:'S: Splinting', color:'teal', icon:'bone', group:'MARCH PAWS Sequence',
    keywords:'splinting evacuation fracture femur traction splint spine precautions medevac 9-line dd1380',
    render() { return `
  ...
  // ── DRUGS ─────────────────────────────────────────────────────────────────
  {
    id:'DRUGS', label:'℞ Drugs', color:'red', icon:'list-ordered', group:'Reference',
    keywords:'drug reference cards dose txa ketamine esketamine suzetrigine ceftriaxone cefadroxil cephalexin midazolam',
    render() { return `
...
// ── Navigation (sidebar groups, homepage tool-list styling; mirrors abx2) ────

let currentId = null;

function buildNav() {
  const q = document.getElementById('tcccSearch').value.trim().toLowerCase();
  const nav = document.getElementById('tabNav');
  nav.innerHTML = '';

  // Preserve declared group order.
  const groups = [];
  ...
  for (const g of groups) {
    ...
    const hasActive = items.some(s => s.id === currentId);
    ...
    hdr.innerHTML = `<span class="nav-section-label">${esc(g)}</span><span class="nav-chevron"></span>`;
    ...
    for (const s of items) {
      ...
      btn.dataset.id = s.id;
      btn.innerHTML = `<i data-lucide="${s.icon}" class="icon-tab"></i><span>${esc(s.label)}</span>`;
...
document.getElementById('tcccSearch').addEventListener('input', buildNav);
buildNav();

// ── Offline badge (online/offline events; no cached content for this test page) ─
